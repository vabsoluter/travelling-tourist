	...
	L.Control.Geocoder = L.Control.extend({
		...
		onAdd: function (map) {
			var className = 'geocoder',
				...
				icon = L.DomUtil.create('div', 'geocoder-icon', container),
			...
			this._alts = L.DomUtil.create('ul', className + '-alternatives geocoder-alternatives-minimized');
		...
		_geocodeResult: function (results) {
			L.DomUtil.removeClass(this._container, 'geocoder-throbber');
			if (results.length === 1) {
				this._geocodeResultSelected(results[0]);
			} else if (results.length > 0) {
				this._alts.innerHTML = '';
				this._results = results;
				L.DomUtil.removeClass(this._alts, 'geocoder-alternatives-minimized');
				for (var i = 0; i < results.length; i++) {
					this._alts.appendChild(this._createAlt(results[i], i));
				}
			} else {
				L.DomUtil.addClass(this._errorElement, 'geocoder-error');
			}
		},
		...
		_geocode: function(event) {
			...
			L.DomUtil.addClass(this._container, 'geocoder-throbber');
			this._clearResults();
		...
		_toggle: function() {
			if (this._container.className.indexOf('geocoder-expanded') >= 0) {
				this._collapse();
			} else {
				this._expand();
			}
		...
		_expand: function () {
			L.DomUtil.addClass(this._container, 'geocoder-expanded');
			this._input.select();
		},

		_collapse: function () {
			this._container.className = this._container.className.replace(' geocoder-expanded', '');
			L.DomUtil.addClass(this._alts, 'geocoder-alternatives-minimized');
			L.DomUtil.removeClass(this._errorElement, 'geocoder-error');
		},
		...
		_keydown: function(e) {
			var _this = this,
				select = function select(dir) {
					if (_this._selection) {
						L.DomUtil.removeClass(_this._selection.firstChild, 'geocoder-selected');
					...
					if (_this._selection) {
						L.DomUtil.addClass(_this._selection.firstChild, 'geocoder-selected');
					}
				};
		...
		options: {
			...
			htmlTemplate: function(r) {
				...
				if (a.city || a.town || a.village) {
					parts.push('<span class="' + (parts.length > 0 ? 'geocoder-address-detail' : '') +
						'">{postcode} {city} {town} {village}</span>');
				}

				if (a.state || a.country) {
					parts.push('<span class="' + (parts.length > 0 ? 'geocoder-address-context' : '') +
						'">{state} {country}</span>');
				}
